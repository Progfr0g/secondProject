...
from src.models import JSONModel, JSONModel2, YAMLModel
...
def yaml_to_json(yaml_filepath, json_filepath):
    ...
    f.close()
    yaml_mod = YAMLModel(**yaml_data)

    json_mod = JSONModel2.from_config_model(yaml_mod.dict())
...
if __name__ == "__main__":
    ...
    output_json_path = (
        sys.argv[2]
        if len(sys.argv) > 2
        else f"./{input_yaml_path.replace('.yaml', '')}_output.json"
    )
